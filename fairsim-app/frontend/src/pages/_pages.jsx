...
import { getHeatmap, getShap, applyFix, eli5Explain, whatifPredict, getReport, getModelDownloadUrl, getPredictionsDownloadUrl, getModelInfo } from '../utils/api'
...
export function HeatmapPage() {
  ...
  useEffect(() => {
    ...
    setLoad(true)
    getHeatmap(sessionId).then(r=>{setData(r);setHeatmap(r)}).catch(e=>toast.error(String(e))).finally(()=>setLoad(false))
  }, [sessionId])
  const cellColor = v => v<30?'#EF4444':v<55?'#F59E0B':'#22C55E'
  return (
    <div style={{maxWidth:740}}>
      <PageHeader tag="Bias Heatmap" title="Group × Metric Fairness Matrix" sub="Each cell is a fairness score (0–100). Red = high bias, amber = medium, green = fair."/>
...
export function FixPage() {
  ...
  const before=fixResult?.before?.composite, after=fixResult?.after?.composite
  return (
    <div style={{maxWidth:660}}>
      ...
      {trainInfo&&(
        <>
          ...
          {loading&&<Spinner text="Retraining with fairness constraints..."/>}
          {fixResult&&!loading&&(
            <div style={{marginTop:12}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
                ...
                <Stat label="Improvement"       value={`+${(after||0)-(before||0)} pts`} color="green"/>
              </div>
              <Btn onClick={()=>nav('/whatif')} fullWidth style={{marginTop:4}}>Try What-If Simulator</Btn>

              {/* ── Download section ── */}
              <div style={{marginTop:16,padding:'14px 16px',background:T.bg,borderRadius:8,border:`1px solid ${T.border}`}}>
                <div style={{fontSize:11,color:T.hint,textTransform:'uppercase',letterSpacing:'0.09em',marginBottom:10}}>Download & use the fixed model</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10}}>
                  <a href={getModelDownloadUrl(sessionId)} download style={{textDecoration:'none'}}>
                    <div style={{border:`1px solid ${T.border}`,borderRadius:7,padding:'10px 12px',cursor:'pointer',background:T.card,transition:'border-color .15s'}}
                         onMouseOver={e=>e.currentTarget.style.borderColor='#6366F1'}
                         onMouseOut={e=>e.currentTarget.style.borderColor=T.border}>
                      <div style={{fontSize:12,fontWeight:500,color:T.text,marginBottom:2}}>Fixed model (.joblib)</div>
                      <div style={{fontSize:11,color:T.hint}}>Load directly in Python with joblib</div>
                    </div>
                  </a>
                  <a href={getPredictionsDownloadUrl(sessionId)} download style={{textDecoration:'none'}}>
                    <div style={{border:`1px solid ${T.border}`,borderRadius:7,padding:'10px 12px',cursor:'pointer',background:T.card,transition:'border-color .15s'}}
                         onMouseOver={e=>e.currentTarget.style.borderColor='#22C55E'}
                         onMouseOut={e=>e.currentTarget.style.borderColor=T.border}>
                      <div style={{fontSize:12,fontWeight:500,color:T.text,marginBottom:2}}>Debiased predictions (.csv)</div>
                      <div style={{fontSize:11,color:T.hint}}>Original vs fixed decisions, side by side</div>
                    </div>
                  </a>
                </div>
                <div style={{background:'#0a0f1a',borderRadius:6,padding:'10px 12px',fontFamily:'monospace',fontSize:11,color:'#94A3B8',lineHeight:1.7}}>
                  <span style={{color:'#64748B'}}># Use the fixed model in your Python code</span><br/>
                  ...
                  predictions = model.<span style={{color:'#22D3EE'}}>predict</span>(your_dataframe)
                </div>
              </div>
            </div>
          )}
...
export function ReportPage() {
  ...
  const nav = useNavigate()
  useEffect(()=>{if(!sessionId)return;setLoad(true);getReport(sessionId).then(setReport).catch(()=>{}).finally(()=>setLoad(false))},[sessionId])
  const before=trainInfo?.fairscore?.composite, after=fixResult?.after?.composite
  return (
    <div style={{maxWidth:760}}>
        ...
        <>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
            ...
            <Card>
              ...
              {fixResult
                ?[['Fix type',fixResult.fix_type?.replace(/_/g,' ')??'—'],['Before',before??'—'],['After',after??'—'],['Improvement',`+${(after||0)-(before||0)} pts`],['Accuracy',`${fixResult.accuracy}%`]].map(([l,v])=>(
                    <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:`1px solid ${T.line}`,fontSize:12}}>
          ...
          <Card style={{marginTop:4}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              <a href={getModelDownloadUrl(sessionId)} download style={{textDecoration:'none'}}>
                <div style={{border:`1px solid ${T.border}`,borderRadius:7,padding:'12px',cursor:'pointer',background:T.bg}}>
                  ...
                  <div style={{fontSize:10,color:'#6366F1',fontWeight:500}}>Download .joblib</div>
                </div>
              </a>
              <a href={getPredictionsDownloadUrl(sessionId)} download style={{textDecoration:'none'}}>
                <div style={{border:`1px solid ${T.border}`,borderRadius:7,padding:'12px',cursor:'pointer',background:T.bg}}>
                  ...
                  <div style={{fontSize:10,color:'#22C55E',fontWeight:500}}>Download .csv</div>
                </div>
              </a>
            </div>
            <div style={{marginTop:10,padding:'10px 12px',background:'#0a0f1a',borderRadius:6,fontFamily:'monospace',fontSize:11,color:'#94A3B8',lineHeight:1.7}}>
              <span style={{color:'#64748B'}}># How to use the downloaded model</span><br/>
              ...
              predictions = model.<span style={{color:'#22D3EE'}}>predict</span>(df[[<span style={{color:'#86EFAC'}}>...required_cols...</span>]])
            </div>
          </Card>
        </>
      )}
